...
class VScope2(QtWidgets.QMainWindow):
    ...
    def _update_readback_boxes(self):
        # only update channels that are in read mode (writeEn unchecked)
    ...
    def _update_legend_overlays(self):
    ...
    def _build_ui(self):
        ...
        self.btn_run.setEnabled(True)  # Run implies Bind
    ...
    def _init_plot(self, plotw, title: str):
        ...
        # NEW: overlay legend text item (one per plot)
    ...
    def _map_save(self):
        ...
        base = self._sanitize_name(self.save_name.text())
        prefix = base if base else "VScope"
        filename= os.path.join(os.getcwd(), "Images", prefix + ".txt")
        if not filename:
            return
        try:
    ...
    def _update_sps_filter(self):
        ...
        alpha = 0.08 # 0..1 (higher = faster reaction)
    ...
    def _plot_tick(self):
        ...
        n = data.shape[0]
        x_samples = np.arange(n, dtype=np.float32)
...
def main():
    ...
    w = VScope2(pg)
    icon_path = os.path.join("Images", "Vscope.ico")
